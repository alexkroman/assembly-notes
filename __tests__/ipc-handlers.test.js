...
describe('IPC Handlers', () => {
  ...
  describe('setupIpcHandlers', () => {
    ...
    it('should handle log messages from renderer', () => {
      ...
      const logHandler = ipcMain.on.mock.calls.find(
        (call) => call[0] === 'log'
      )[1];
    ...
    it('should handle microphone audio data', () => {
      const {
        sendMicrophoneAudio,
      } = require('../src/main/recordingManager.js');

      setupIpcHandlers(mockMainWindow);

      // Find the microphone audio handler
      const micHandler = ipcMain.on.mock.calls.find(
        (call) => call[0] === 'microphone-audio-data'
      )[1];
    ...
    it('should handle system audio data', () => {
      const { sendSystemAudio } = require('../src/main/recordingManager.js');

      setupIpcHandlers(mockMainWindow);

      // Find the system audio handler
      const sysHandler = ipcMain.on.mock.calls.find(
        (call) => call[0] === 'system-audio-data'
      )[1];
    ...
    it('should handle start-recording request', async () => {
      const { startTranscription } = require('../src/main/recordingManager.js');

      setupIpcHandlers(mockMainWindow);

      // Find the start-recording handler
      const startHandler = ipcMain.handle.mock.calls.find(
        (call) => call[0] === 'start-recording'
      )[1];
    ...
    it('should handle stop-recording request', async () => {
      const { stopTranscription } = require('../src/main/recordingManager.js');

      setupIpcHandlers(mockMainWindow);

      // Find the stop-recording handler
      const stopHandler = ipcMain.handle.mock.calls.find(
        (call) => call[0] === 'stop-recording'
      )[1];
    ...
    it('should handle install-update request', () => {
      ...
      const installHandler = ipcMain.handle.mock.calls.find(
    ...
    it('should handle quit-and-install request', () => {
      ...
      const quitHandler = ipcMain.handle.mock.calls.find(
    ...
    it('should handle check-for-updates request', () => {
      ...
      } = require('../src/main/auto-updater.js');

      setupIpcHandlers(mockMainWindow);

      // Find the check-for-updates handler
      const checkHandler = ipcMain.handle.mock.calls.find(
