...
import { AudioContext, AudioWorkletNode, mockAudioContext, mockWorkletNode, mockMediaStreamSource, } from '../../__mocks__/audio-context';
...
describe('AudioProcessing Module', () => {
    ...
    describe('startAudioProcessing', () => {
        ...
        it('should handle microphone audio data messages', async () => {
            ...
            expect(window.electronAPI.sendMicrophoneAudio).toHaveBeenCalledWith(audioData);
        });
        it('should initialize system audio processing when provided', async () => {
            ...
            await AudioProcessing.startAudioProcessing(mockMicStream, mockSystemStream);
        ...
        it('should handle system audio data messages', async () => {
            ...
            const systemWorkletNode = AudioWorkletNode.mock.results[1]
                .value;
            const audioData = new Int16Array([4, 5, 6]);
            systemWorkletNode.port.onmessage({
                data: { type: 'audioData', data: audioData },
            });
            expect(window.electronAPI.sendSystemAudio).toHaveBeenCalledWith(audioData);
        });
    });
