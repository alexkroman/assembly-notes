...
window.electronAPI.onStartAudioCapture(() => {
  logger.info('Renderer: Received start-audio-capture event');
  void (async () => {
    try {
...
window.electronAPI.onResetAudioProcessing(() => {
  try {
    ...
    logger.error('Error resetting audio processing:', error);
  }
});
...
try {
  ...
  logger.error('Error initializing React app:', error);
